...
export default class ItemRemito extends Component {
   ...
   getDistance = (destination) => {
      ...
      const a =
         ...
         Math.cos(this.deg2rad(this.props.latitud)) * Math.cos(this.deg2rad(destination.latitud)) *
   ...
   render() {
      ...
      const tempDist = this.getDistance({ latitud: item.latitud, longitud: item.longitud })
      // format distance
      const distance = parseFloat(Math.round(tempDist * 100) / 100).toFixed(2) + ' kms';

      return (

         <View style={styles.shadow}>

            <TouchableOpacity style={styles.listitem} onPress={this.props.onPressSingleItem}>
